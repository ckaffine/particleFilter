...
def cached_x(f):
    cache = {}
    def wrapper(self, x):
        key = (int(x[0] / 10), int(x[1] / 10), int(x[2] / math.radians(5)))
        if key in cache:
            return cache[key]
        else:
            cache[key] = f(self, x)
...
class SensorModel:
    ...
    def beam_range_finder_model(self, z_t1_arr, x_t1):
        ...
        for i, t in enumerate(np.linspace(th - math.pi / 2,
                                          th + math.pi / 2, 
                                          num=num)):
            pos = np.array([laser_x, laser_y, t])
            z_stars[i] = self.beam_range_measurement(pos)
